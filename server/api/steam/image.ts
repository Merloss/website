...
const fullKey = (id: string) => `steam/${id}.jpg`;
const thumbKey = (id: string) => `steam/${id}_thumb.jpg`;
...
export default defineEventHandler(async (event) => {
  ...
  const full = q.size === "full";
  const key = full ? fullKey(id) : thumbKey(id);

  // 1. Already in R2 → serve it. Steady state is zero Steam traffic.
  let bytes = await r2GetImage(key);
  ...
  if (!bytes) {
    const flightKey = `${id}:${full ? "f" : "t"}`;
    let task = migrating.get(flightKey);
    if (!task) {
      task = (async () => {
        const base = await resolveBase(id);
        if (!base) return null;
        const img = await downloadImage(sizedUrl(base, full ? 5000 : 600));
